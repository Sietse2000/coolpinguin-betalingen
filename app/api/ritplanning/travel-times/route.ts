...
export async function POST(req: NextRequest) {
  ...
  if (!addresses || addresses.length === 0) {
    return NextResponse.json({ depot: env.DEPOT_ADDRESS, pairs: {}, distancePairs: {} })
  }
  ...
  if (data.status !== 'OK') {
    ...
    return NextResponse.json({ depot, pairs: {}, distancePairs: {} })
  }

  const pairs: Record<string, number> = {}
  const distancePairs: Record<string, number> = {}
  for (let i = 0; i < unique.length; i++) {
    ...
    for (let j = 0; j < unique.length; j++) {
      ...
      if (el?.status === 'OK') {
        ...
        distancePairs[`${unique[i]}|${unique[j]}`] = Math.round(el.distance.value / 100) / 10 // km (1 decimaal)
      }
    }
  }
